...
void test(std::vector<test_case<word_t, words>> cases)
{
  for (auto x : cases)
  {
    auto bijection = make_threefry_functor<word_t, words, rounds>(x.key);
...
{
  test<uint64_t, 4, 13>({{{0x243f6a8885a308d3, 0x13198a2e03707344, 0xa4093822299f31d0, 0x082efa98ec4e6c89},
  ...
  test<uint64_t, 4, 20>({{{0x243f6a8885a308d3, 0x13198a2e03707344, 0xa4093822299f31d0, 0x082efa98ec4e6c89},
  ...
  test<uint64_t, 4, 72>({{{0x243f6a8885a308d3, 0x13198a2e03707344, 0xa4093822299f31d0, 0x082efa98ec4e6c89},
                          {0x452821e638d01377, 0xbe5466cf34e90c6c, 0xbe5466cf34e90c6c, 0xc0ac29b7c97c50dd},
  ...
  test<uint64_t, 2, 13>(
      {{{0x243f6a8885a308d3, 0x13198a2e03707344},
       ...
       {{UINT64_MAX, UINT64_MAX}, {UINT64_MAX, UINT64_MAX}, {0xccdec5c917a874b1, 0x4df53abca26ceb01}}});
  ...
  test<uint64_t, 2, 20>(
      {{{0x243f6a8885a308d3, 0x13198a2e03707344},
       ...
       {{UINT64_MAX, UINT64_MAX}, {UINT64_MAX, UINT64_MAX}, {0xe02cb7c4d95d277a, 0xd06633d0893b8b68}}});
  ...
  test<uint64_t, 2, 32>(
      {{{0x243f6a8885a308d3, 0x13198a2e03707344},
       ...
       {{UINT64_MAX, UINT64_MAX}, {UINT64_MAX, UINT64_MAX}, {0x6b532f4f6e288646, 0x0388f1ec135ee18e}}});

  test<uint32_t, 4, 13>({{{0x243f6a88, 0x85a308d3, 0x13198a2e, 0x03707344},
  ...
  test<uint32_t, 4, 20>({{{0x243f6a88, 0x85a308d3, 0x13198a2e, 0x03707344},
                          {0xa4093822, 0x299f31d0, 0x082efa98, 0xec4e6c89},
  ...
  test<uint32_t, 4, 72>({{{0x243f6a88, 0x85a308d3, 0x13198a2e, 0x03707344},
                          {0xa4093822, 0x299f31d0, 0x082efa98, 0xec4e6c89},
  ...
  test<uint32_t, 2, 13>({{{0x243f6a88, 0x85a308d3}, {0x13198a2e, 0x03707344}, {0xba3e4725, 0xf27d669e}},
                         {{}, {}, {0x9d1c5ec6, 0x8bd50731}},
                         {{UINT32_MAX, UINT32_MAX}, {UINT32_MAX, UINT32_MAX}, {0xfd36d048, 0x2d17272c}}});

  test<uint32_t, 2, 20>({{{0x243f6a88, 0x85a308d3}, {0x13198a2e, 0x03707344}, {0xc4923a9c, 0x483df7a0}},
                         {{}, {}, {0x6b200159, 0x99ba4efe}},
                         {{UINT32_MAX, UINT32_MAX}, {UINT32_MAX, UINT32_MAX}, {0x1cb996fc, 0xbb002be7}}});

  test<uint32_t, 2, 32>({{{0x243f6a88, 0x85a308d3}, {0x13198a2e, 0x03707344}, {0xe2827716, 0xc3c05cdf}},
                         {{}, {}, {0xcee3d47e, 0xa23dfd5c}},
                         {{UINT32_MAX, UINT32_MAX}, {UINT32_MAX, UINT32_MAX}, {0x6e2fe0d0, 0xb1b76f82}}});
}

template <class word_t, size_t words, unsigned rounds, counter<word_t, words> key>
void test()
{
  auto bijection1 = make_threefry_functor<word_t, words, rounds>(key);
  auto bijection2 = make_threefry_functor<word_t, words, rounds, key>();
...
void compare_runtime_and_compile_time_key()
{
  ...
  test<uint64_t, 4, 20, {1, 2, 3, 4}>();
  test<uint32_t, 2, 20, {5, 6}>();
  test<uint64_t, 4, 20, {7, 8, 9, 1}>();
  test<uint32_t, 2, 20, {2, 3}>();
...
{
  constexpr auto bijection = make_threefry_functor<uint64_t, 4, 72>({});

  [[maybe_unused]]
  constexpr auto result = bijection({});
}
