...
auto test(std::vector<test_case<word_t, words>> cases)
{
  for (auto x : cases)
  {
    auto bijection = make_philox_functor<word_t, words, rounds>(x.key);
...
{
  test<uint64_t, 4, 7>({{{0x243f6a8885a308d3, 0x13198a2e03707344, 0xa4093822299f31d0, 0x082efa98ec4e6c89},
  ...
  test<uint64_t, 4, 10>({{{0x243f6a8885a308d3, 0x13198a2e03707344, 0xa4093822299f31d0, 0x082efa98ec4e6c89},
                          {0x452821e638d01377, 0xbe5466cf34e90c6c},
  ...
  test<uint64_t, 2, 7>(
      {{{0x243f6a8885a308d3, 0x13198a2e03707344}, {0xa4093822299f31d0}, {0x98ed1534392bf372, 0x67528b1568882fd5}},
       {{}, {}, {0xb41da69fbfefc666, 0x511e9ce1a5534056}},
       {{UINT64_MAX, UINT64_MAX}, {UINT64_MAX}, {0xa4696cc04462015d, 0x724782dae17169e9}}});

  test<uint64_t, 2, 10>(
      {{{0x243f6a8885a308d3, 0x13198a2e03707344}, {0xa4093822299f31d0}, {0x0a5e742c2997341c, 0xb0f883d38000de5d}},
       {{}, {}, {0xca00a0459843d731, 0x66c24222c9a845b5}},
       {{UINT64_MAX, UINT64_MAX}, {UINT64_MAX}, {0x65b021d60cd8310f, 0x4d02f3222f86df20}}});

  test<uint32_t, 4, 7>({{{0x243f6a88, 0x85a308d3, 0x13198a2e, 0x03707344},
  ...
  test<uint32_t, 4, 10>({{{0x243f6a88, 0x85a308d3, 0x13198a2e, 0x03707344},
                          {0xa4093822, 0x299f31d0},
  ...
  test<uint32_t, 2, 7>({{{0x243f6a88, 0x85a308d3}, {0x13198a2e}, {0xbedbbe6b, 0xe4c770b3}},
                        {{}, {}, {0x257a3673, 0xcd26be2a}},
                        {{UINT32_MAX, UINT32_MAX}, {UINT32_MAX}, {0xab302c4d, 0x3dc9d239}}});

  test<uint32_t, 2, 10>({{{}, {}, {0xff1dae59, 0x6cd10df2}},
                         {{UINT32_MAX, UINT32_MAX}, {UINT32_MAX}, {0x2c3f628b, 0xab4fd7ad}},
                         {{0x243f6a88, 0x85a308d3}, {0x13198a2e}, {0xdd7ce038, 0xf62a4c12}}});
}

template <class word_t, size_t words, unsigned rounds, counter<word_t, words / 2> key>
void test()
{
  auto bijection1 = make_philox_functor<word_t, words, rounds>(key);
  auto bijection2 = make_philox_functor<word_t, words, rounds, key>();
...
{
  constexpr auto bij = make_philox_functor<uint64_t, 4, 16>({});

  [[maybe_unused]]
  constexpr auto result = bij({});
}
